...
(async () => {
  const url = process.argv[2] || "http://localhost:3000/dev/templates/tpl_2";
  ...
  try {
    ...
    const data = await page.evaluate((pxPerPt) => {
      ...
      const headings = qa(".tpl-2-heading, .tpl-tl-heading");
      ...
      return {
        ...
        badge: {
          x: rel(q(".tpl-2-badge"), "x"),
          y: rel(q(".tpl-2-badge"), "y"),
          right: rel(q(".tpl-2-badge"), "right"),
          bottom: rel(q(".tpl-2-badge"), "bottom"),
          color: cs(q(".tpl-2-badge"), "backgroundColor"),
          text: q(".tpl-2-badge")?.textContent,
        },
        name: {
          x: rel(q(".tpl-2-name"), "x"),
          y: rel(q(".tpl-2-name"), "y"),
          bottom: rel(q(".tpl-2-name"), "bottom"),
          size: cs(q(".tpl-2-name"), "fontSize"),
          color: cs(q(".tpl-2-name"), "color"),
          weight: cs(q(".tpl-2-name"), "fontWeight"),
        },
        firstHeading: {
        ...
        rowDisplay: cs(q(".tpl-tl-row"), "display"),
      };
    }, PX_PER_PT);
