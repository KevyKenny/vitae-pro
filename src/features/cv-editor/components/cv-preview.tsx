...
/** Preview is always A4 (210 × 297 mm). */
...
export function CVPreview({ className }: { className?: string }) {
  const { document, zoom, setZoom, customization } = useEditor();
  ...
  return (
    <div className={cn("flex h-full min-h-0 w-full flex-col bg-paper-dim", className)}>
