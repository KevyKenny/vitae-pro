/** Map Supabase / template persistence errors to user-facing copy. */
export function templateErrorMessage(error: unknown, fallback?: string): string {
  ...
  if (
    ...
    return "You don't have permission to access templates.";
  }
  if (
    ...
    return "We couldn't find that template.";
  }
  if (
  ...
  if (
    ...
    return fallback ?? "We couldn't save your template settings. Please try again.";
  }
